"""Export RSS entries to JSON for the demo web UI.

What this script does:
    - reads entries from the SQLite database
    - writes them to ``web/basic/data/entries.json``

Usage:
    python scripts/python/export_json.py
    python scripts/python/export_json.py --db level2-aggregator/db.sqlite
    python scripts/python/export_json.py --out web/basic/data/entries.json
...
DEFAULT_OUTPUT_PATH = "web/basic/data/entries.json"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Export SQLite entries to JSON",
    )
    parser.add_argument(
        "--db",
